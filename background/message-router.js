  ...
  function createMessageRouter(deps = {}) {
    ...
    async function handleMessage(message, sender) {
      switch (message.type) {
        ...
        case 'UPSERT_GMAIL_CUSTOM_POOL_ACCOUNT': {
          const account = await upsertGmailCustomPoolAccount(message.payload || {});
          return { ok: true, account };
        }

        case 'DELETE_GMAIL_CUSTOM_POOL_ACCOUNT': {
          await deleteGmailCustomPoolAccount(String(message.payload?.accountId || ''));
          return { ok: true };
        }

        case 'DELETE_GMAIL_CUSTOM_POOL_ACCOUNTS': {
          const result = await deleteGmailCustomPoolAccounts(String(message.payload?.mode || 'all'));
          return { ok: true, ...result };
        }

        case 'SELECT_GMAIL_CUSTOM_POOL_ACCOUNT': {
          const account = await setCurrentGmailCustomPoolAccount(String(message.payload?.accountId || ''), {
            syncEmail: true,
          });
          return { ok: true, account };
        }

        case 'PATCH_GMAIL_CUSTOM_POOL_ACCOUNT': {
          const account = await patchGmailCustomPoolAccount(
            String(message.payload?.accountId || ''),
            message.payload?.updates || {}
          );
          return { ok: true, account };
