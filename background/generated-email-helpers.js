(function attachGeneratedEmailHelpers(root, factory) {
  ...
  function createGeneratedEmailHelpers(deps = {}) {
    ...
    async function fetchGmailCustomPoolEmail(state, options = {}) {
      throwIfStopped();
      const latestState = state || await getState();
      if (typeof ensureGmailCustomPoolAccountForFlow !== 'function') {
        throw new Error('Gmail 自定义号池能力尚未接入。');
      }

      const account = await ensureGmailCustomPoolAccountForFlow({
        allowAllocate: true,
        preferredAccountId: options.currentGmailCustomPoolAccountId ?? latestState.currentGmailCustomPoolAccountId ?? null,
        syncEmail: false,
      });
      await setEmailState(account.email);
      await addLog(`Gmail 自定义号池：已使用 ${account.email}`, 'ok');
    ...
    async function fetchGeneratedEmail(state, options = {}) {
      ...
      const gmailRegistrationMode = options.gmailRegistrationMode !== undefined
        ? options.gmailRegistrationMode
        : currentState.gmailRegistrationMode;
      if (typeof isGmailCustomPoolMode === 'function' && isGmailCustomPoolMode({
        ...currentState,
        mailProvider: provider,
        gmailRegistrationMode,
      })) {
        return fetchGmailCustomPoolEmail(currentState, options);
      }
      if (isGeneratedAliasProvider?.(provider, mail2925Mode, gmailRegistrationMode)) {
