...
test('managed alias utils disable gmail alias generation in custom pool mode', () => {
  assert.equal(api.normalizeGmailRegistrationMode('alias'), 'alias');
  assert.equal(api.normalizeGmailRegistrationMode('custom_pool'), 'custom_pool');
  assert.equal(api.normalizeGmailRegistrationMode('other'), 'alias');
  assert.equal(api.usesManagedAliasGeneration('gmail', { gmailRegistrationMode: 'alias' }), true);
  assert.equal(api.usesManagedAliasGeneration('gmail', { gmailRegistrationMode: 'custom_pool' }), false);
});
