...
test('signup flow helper uses gmail custom pool account when gmail is in custom pool mode', async () => {
  const ensureCalls = [];
  const setEmailCalls = [];
  ...
  const helpers = signupFlowApi.createSignupFlowHelpers({
    ...
    ensureGmailCustomPoolAccountForFlow: async (options) => {
      ...
      return { id: 'pool-7', email: 'pool.user@example.net' };
    },
    ensureHotmailAccountForFlow: async () => {
  ...
  const email = await helpers.resolveSignupEmailForFlow({
    ...
    currentGmailCustomPoolAccountId: 'pool-7',
    email: '',
  });
  ...
  assert.deepStrictEqual(ensureCalls, [
    {
      allowAllocate: true,
      preferredAccountId: 'pool-7',
    },
  ]);
  assert.deepStrictEqual(setEmailCalls, ['pool.user@example.net']);
});
