...
test('generated email helper allocates gmail custom pool email before any generator fallback', async () => {
  const api = loadGeneratedEmailHelpersApi();
  const events = [];
  ...
  const helpers = api.createGeneratedEmailHelpers({
    ...
    getState: async () => ({
      ...
      currentGmailCustomPoolAccountId: 'pool-2',
      emailGenerator: 'duck',
    }),
    ensureGmailCustomPoolAccountForFlow: async (options) => {
      events.push(['ensure', options]);
      return { id: 'pool-2', email: 'pool.user@example.net' };
    },
    ensureMail2925AccountForFlow: async () => {
    ...
    sendToContentScript: async () => {
      throw new Error('should not message duck tab');
    },
    setEmailState: async (email) => {
      events.push(['email', email]);
    },
  ...
  assert.equal(email, 'pool.user@example.net');
  assert.deepStrictEqual(events, [
    ['ensure', {
      allowAllocate: true,
      preferredAccountId: 'pool-2',
      syncEmail: false,
    }],
